...
const WalletContextProvider: React.FC = ({ children }) => {
  const [state, setState] = useReducer(
    (prevState: IWalletContextProviderState, newState: Partial<IWalletContextProviderState>) => ({ ...prevState, ...newState }),
    {
      provider: getDefaultProvider(),
      providerExtensionInstalled: !!window.ethereum,
      account: null,
  ...
  useEffect(() => {
    window.ethereum.on("accountsChanged", (accounts?: string[]) => handleAccountChange(accounts?.[0]));
  });
  ...
  return <WalletContext.Provider value={{ ...state, connectWallet }}>{children}</WalletContext.Provider>;
};
