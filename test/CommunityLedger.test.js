...
describe('CommunityLedger', () => {
  ...
  describe('Mint flow', () => {
    it('Should not allow minting if minting is not enabled', async () => {
      await expect(ledger.connect(user1).mint(TOKEN_URI)).to.be.revertedWith(
        'Minting not enabled at the moment'
      )
    })

    it('Should only allow owner enabling minting', async () => {
      expect(await ledger.mintEnabled()).to.equal(false)
      await expect(ledger.connect(user1).setMintingStatus(true)).to.be.revertedWith(
    ...
    it('Should allow owner withdraw total collected mint fee', async () => {
      await expect(ledger.connect(user1).withdrawMintFee()).to.be.revertedWith(
        'Ownable: caller is not the owner'
      )
